...
import { DynamicTableRows } from "../create/tableRows";
...
function InvoiceEdit(props) {
  ...
  const handleChange = (dataToSet, stateToSet) => {
    switch (stateToSet) {
      case "date":
        setStartDate(dataToSet.date._d);
        break;
      case "patient":
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      let path = `${EndPoints.backendProd}/api/Invoices/getInvoiceByID?id=${invoiceParam.documentId}`
      console.log(path);
      try {
        ...
        console.log(taxRes);
        const resTax = invoiceRes.data.Taxes;
        ...
        const resAvailableId = invoiceRes.data.invoiceByID.ID;
        ...
        resPatients.map((resPatient) => {
          PatientsToDisplay.push({ value: resPatient.Name, label: resPatient.Name, _id: resPatient._id, phoneNo: resPatient.Phone })
          PhoneNoToDisplay.push({ value: resPatient.Phone, label: resPatient.Phone, patientName: resPatient.Name, _id: resPatient._id })
        })
        ...
        setTotalAmount(resInvoiceData.Amount)
        setStartDate(resInvoiceData.CreatedDate)
        setTotalDiscount(parseInt(resInvoiceData.Discount))
        setSelectedRole(resInvoiceData.Role ? { value: 'Paid', label: 'Paid', boolValue: true } : { value: 'Unpaid', label: 'Unpaid', boolValue: false })
        setRowsData(resInvoiceData.Items)
        setOtherInfo(resInvoiceData.OtherInformation)
        setSelectedPatient({ value: resInvoiceData.Patient.Name, label: resInvoiceData.Patient.Name, _id: resInvoiceData.Patient._id, phoneNo: resInvoiceData.Patient.Phone })
        setPhoneNo({ value: resInvoiceData.Patient.Phone, label: resInvoiceData.Patient.Phone, patientName: resInvoiceData.Patient.Name, _id: resInvoiceData.Patient._id })
        setSelectedDoctor({ value: resInvoiceData.Doctor.Name, label: resInvoiceData.Doctor.Name, _id: resInvoiceData.Doctor._id, department: resInvoiceData.Doctor.Department.Name })
        seTSelectedDepartment(resInvoiceData.Doctor.Department.Name);
      } catch (error) {
        console.log(error)
        return;
      }
  ...
  const handleChangeTable = (index, value, caller) => {
    ...
    switch (caller) {
      ...
      case "totalDiscount":
        ...
        setTotalDiscount(value > totalDiscount ? totalDiscount + 2 : totalDiscount - 2)
        break;
      default:
    }
  ...
  const handleSaveInvoice = async (e) => {
    ...
    const invoiceParam = location.invoice;
    const headers = { "Content-Type": "application/json", "Access-Control-Allow-Origin": "*","username":AuthHandler.getUser().username };
    const path = `${EndPoints.backendProd}/api/Invoices/editInvoice`
    console.log(path);
    try {
      ...
      await axios.put(path,
        {
          _id: invoiceParam.documentId,
          id: selectedInvoiceId, patientId: selectedPatient._id,
  ...
  return (
    <div className="page-wrapper">
      {loading ?
        <div style={{ "background": "white", "height": "100%", "width": "100%", position: "fixed", "zIndex": 999, "paddingTop": "44vh", "paddingBottom": "44vw", "paddingRight": "44vh", "paddingLeft": "44vw" }}>
          <ThreeDots
            visible={true}
            height="80"
            width="80"
                ...
                <div className="row">
                  {/* <div className="col-sm-6 col-md-3">
                    <div className="form-group">
                      ...
                      <input value={selectedInvoiceId ? `ACLINV-${selectedInvoiceId.toString().padStart(3, '0')}` : ""} disabled={true} className="form-control" />
                    </div>
                  </div> */}
                  <div className="col-sm-6 col-md-3">
                    <div className="form-group">
                  ...
                  <div className="col-sm-6 col-md-3">
                    <div className="form-group">
                      ...
                      <Select value={selectedDoctor} onChange={(e) => { handleChange(e, "doctor") }} options={doctorsToDisplay} className="select" />
                    </div>
                  </div>
                  <div className="col-sm-6 col-md-3">
                    <div className="form-group">
                ...
                <div className="row">
                  <div className="col-md-12 col-sm-12">
                    ...
                    <div className="text-center m-t-20">
                      <button onClick={handleSaveInvoice} className="btn btn-primary submit-btn">Edit Invoice</button>
                    </div>
                  </div>
                </div>
                {/*row */}
